...
class Wantoitem(models.Model):
    ...
    def scraping(self):
        ...
        kw_in_title_path = './ca_camera/pattern/ca_camera/kw_in_title.txt'
        kw_out_title_path = './ca_camera/pattern/ca_camera/kw_out_title.txt'
        kw_in_list = def_chrome.kw_in_title(kw_in_title_path)
        kw_out_list = def_chrome.kw_out_title(kw_out_title_path)
        search_word = self.maker_name.name+' AND '+self.item_name+' AND '+('({})'+' -{}').format(kw_in_list,kw_out_list)
        def_chrome.search(driver, search_word)
        except_file_main = './ca_camera/pattern/ca_camera/except_main_list.txt'
        except_file_sub = './ca_camera/pattern/ca_camera/except_sub_list.txt'
        contain_title = './ca_camera/pattern/ca_camera/contain_title.txt'
        except_title = './ca_camera/pattern/ca_camera/except_title.txt'
